...
On the complete 142-feature set, the selection protocol (highest fold-9 F1) selected logistic regression. Logistic regression achieved an F1 score of 0.750 on the test fold. Random Forest was not selected by the protocol, but it still achieved the highest test-fold F1 and is reported as the best-observed model. The numbers below are for Random Forest at the default threshold of 0.5, and no class weighting.
